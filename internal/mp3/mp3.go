...
func ReencodeMP3(inputPath, outputPath string, bitRate, sampleRate int, progressCallback ProgressCallback) error {
    ...
    wavFile := &wav.Encoder{
        ...
        ChannelCount:     mp3Decoder.Channels(),
        BitsPerSample:    16,
        AudioFormat:      1, // PCM
        BlockAlign:       0,
        ByteRate:         0,
        DataSize:         0,
        FormatTag:        0,
        ExtraFormatBytes: nil,
        AudioWriter:      &progressWriter{outputPath: outputPath, progressCallback: progressCallback},
    }

    // Start encoding
    buf := &audio.IntBuffer{}
    for {
        if err := mp3Decoder.Decode(buf); err != nil {
            if err == io.EOF {
                break
            }
            return err
        ...
        if sampleRate != mp3Decoder.SampleRate() {
            buf = resample(buf, sampleRate, mp3Decoder.SampleRate())
        }

        // Write the buffer to WAV file
        if err := wavFile.Write(buf); err != nil {
            return err
        }
    }
		    // Finalize WAV file
